...
object UmengClient {
     ...
     * @param listener              分享监听
     */
    fun share(activity: Activity?, platform: Platform?, action: ShareAction?, listener: UmengShare.OnShareListener?) {
        if (platform == null) {
            return
        }
        if (action == null) {
        ...
        action.setPlatform(platform.getThirdParty())
            .setCallback(UmengShare.ShareListenerWrapper(platform.getThirdParty()!!, listener))
            .share()
    }
    ...
    fun login(activity: Activity?, platform: Platform?, listener: UmengLogin.OnLoginListener?) {
        ...
        try {
            ...
            UMShareAPI.get(activity).getPlatformInfo(
                ...
                UmengLogin.LoginListenerWrapper(platform.getThirdParty()!!, listener)
            )
