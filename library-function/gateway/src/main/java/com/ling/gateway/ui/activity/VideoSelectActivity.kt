...
import com.ling.gateway.ui.adapter.VideoSelectAdapter
...
class VideoSelectActivity : AppActivity(), StatusAction, Runnable, BaseAdapter.OnItemClickListener,
    ...
    override fun onRightClick(titleBar: TitleBar?) {
        ...
        for (key: String in keys) {
            ...
            count += list.size
            data.add(AlbumDialog.AlbumInfo(list[0].getVideoPath(), key,
                String.format(getString(R.string.video_select_total),
                    list.size), adapter.getData() === list))
        }
        data.add(0, AlbumDialog.AlbumInfo(allVideo[0].getVideoPath(), getString(R.string.video_select_all),
